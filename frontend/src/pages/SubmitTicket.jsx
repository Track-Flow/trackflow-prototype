...
const CATEGORY_ICON = {
  "it support": {
    icon: "computer",
    color: "#5a8dc4",
    bg: "rgba(90,141,196,0.12)",
  },
  facilities: { icon: "build", color: "#c49a4a", bg: "rgba(196,154,74,0.12)" },
  administration: {
    icon: "business_center",
    color: "#7a6fa8",
    bg: "rgba(122,111,168,0.12)",
  },
  "library services": {
    icon: "menu_book",
    color: "#5a8f72",
    bg: "rgba(90,143,114,0.12)",
  },
        ...
        {categories.map((cat) => {
          ...
          return (
            ...
            >
              ...
              <Box>
                <Typography
                  sx={{ fontSize: 13.5, fontWeight: 600, color: "#e3e8f0" }}
                >
                  {cat.category_name}
                </Typography>
      ...
      <DialogContent sx={{ textAlign: "center", py: 4, px: { xs: 2, sm: 4 } }}>
        <Box
          sx={{
            ...
            display: "grid",
            placeItems: "center",
            bgcolor: "rgba(90,143,114,0.12)",
            border: "2px solid #5a8f72",
          }}
        >
          <span
            className="material-symbols-outlined"
            style={{ fontSize: 32, color: "#5a8f72" }}
          >
            check
          </span>
        </Box>
        ...
        {ticketId && (
          <Typography
            sx={{ fontSize: 12, fontFamily: "monospace", color: ACCENT, mb: 1 }}
          >
            #{ticketId}
          </Typography>
        )}
        <Typography
          sx={{ fontSize: 13, color: "#94a3b8", lineHeight: 1.7, mb: 3 }}
        >
        ...
        <Button
          ...
          sx={{ py: 0.9, color: "#94a3b8", fontSize: 13, mb: 0.5 }}
        >
          View my tickets
        </Button>
        <Button
          fullWidth
          onClick={onAnother}
          sx={{ py: 0.9, color: "#94a3b8", fontSize: 13 }}
        >
          Submit another
        </Button>
      </DialogContent>
    ...
    if (!dupConfirmed) {
      ...
      try {
        ...
        if (res.data.duplicates?.length > 0) {
          ...
          return; // halts here — dialog shown below
        }
      } catch {
    ...
    <Box>
      ...
      >
        <DialogContent sx={{ py: 3, px: { xs: 2, sm: 3 } }}>
          <Typography
            sx={{ fontSize: 16, fontWeight: 700, color: "#e3e8f0", mb: 1 }}
          >
            Possible duplicate
          </Typography>
          <Typography
          ...
          >
            You already have{" "}
            {duplicates.length === 1 ? "an open ticket" : "open tickets"} that
            look similar:
          </Typography>
          <Box sx={{ display: "flex", flexDirection: "column", gap: 1, mb: 3 }}>
            {duplicates.map((d) => (
              ...
              >
                <Typography
                  sx={{ fontSize: 12.5, fontWeight: 600, color: "#e3e8f0" }}
                >
                  #{d.ticket_id} — {d.ticket_title}
                </Typography>
