...
function NoteDialog({ open, mode, onConfirm, onCancel }) {
  ...
  const configByMode = {
    ...
    struggling: {
      ...
      helper:
        "Let the team know what you're stuck on before flagging this ticket.",
      placeholder: "What are you stuck on?",
      confirmLabel: "Flag as Struggling",
      confirmColor: "error",
      confirmIcon: "flag",
    },
    reopen: {
      ...
      title: "Reopen Ticket",
      helper:
        "Explain why this ticket is being reopened. The requester will be notified.",
      placeholder:
        "Reason for reopening (e.g. issue recurred, resolution incomplete)…",
      confirmLabel: "Reopen",
  ...
  confirmIcon: "public",
    ...
    escalate: {
      ...
      helper:
        "Explain why this ticket needs urgent attention. The reason is recorded and other managers will be notified.",
      placeholder:
        "Reason for escalating (e.g. VIP requester, critical system down)…",
      confirmLabel: "Escalate",
      confirmColor: "error",
      confirmIcon: "campaign",
    },
  };
    ...
    >
      ...
      >
        ...
        >
          <span
            className="material-symbols-outlined"
            style={{ fontSize: 15, color, fontVariationSettings: "'FILL' 1" }}
          >
            {icon}
          </span>
        </Box>
        {!isLast && (
          <Box
            sx={{ flex: 1, width: 1, bgcolor: BORDER, mt: 0.5, minHeight: 24 }}
          />
        )}
      </Box>
      <Box sx={{ flex: 1, minWidth: 0, pt: 0.25 }}>
        <Typography
          sx={{ fontSize: 12.5, fontWeight: 700, color: TEXT_BRIGHT, mb: 0.25 }}
        >
    ...
    >
      ...
      >
        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
          <span
            className="material-symbols-outlined"
            style={{ color: "#5a8dc4", fontSize: 20 }}
          >
            history
          </span>
          Ticket lifecycle
      ...
      <DialogContent
        dividers
        sx={{ borderColor: BORDER, px: 2.5, py: 2, maxHeight: "60vh" }}
      >
        {loading && (
        ...
        {!loading && !error && history.length === 0 && (
          <Typography
            sx={{ color: TEXT_DIM, fontSize: 13, textAlign: "center", py: 3 }}
          >
      ...
      <DialogActions sx={{ px: 2.5, py: 1.5, flexShrink: 0 }}>
        <Button onClick={onClose} size="small" sx={{ color: TEXT_DIM }}>
          Close
        </Button>
      </DialogActions>
    </Dialog>
      ...
      {[1, 2, 3, 4, 5].map((star) => (
        <span
          ...
          style={{
            ...
            color: star <= (hover || value) ? '#c49a4a' : 'rgba(148,163,184,0.3)',
  ...
  return (
    <Box sx={{ display: "flex", gap: 1.5, mb: 2.5 }}>
      ...
      <Box sx={{ flex: 1, minWidth: 0 }}>
        ...
        >
          <Typography
            sx={{ fontSize: 12.5, fontWeight: 700, color: TEXT_BRIGHT }}
          >
            {comment.authorName}
          </Typography>
          {isTLA && (
...
const handleDownloadAttachment = async () => {
  try {
    const res = await api.get(
      `/tickets/${ticket.ticket_id}/attachment/download`,
      { responseType: "blob" },
    );
    const url = window.URL.createObjectURL(new Blob([res.data]));
    const link = document.createElement("a");
    link.href = url;
    link.setAttribute("download", ticket.attachment_file_name ?? "attachment");
    ...
    try {
      ...
      setError(
        err.response?.data?.error ??
          "Failed to claim ticket. It may already be claimed.",
      );
    } finally {
      setClaiming(false);
    }
...
const canFlagDepartment =
  ...
  const canEscalate =
    isManager && !ticket.ticket_escalated && !isResolved && !isClosed;
    ...
    <Box>
      ...
      <Button
        ...
        sx={{
          ...
          py: 1.1,
          borderRadius: 2,
          bgcolor: "rgba(90,141,196,0.10)",
          border: "1px solid rgba(90,141,196,0.35)",
          "&:hover": {
            bgcolor: "rgba(90,141,196,0.18)",
            borderColor: "rgba(90,141,196,0.55)",
          },
        }}
      >
      ...
      >
        ...
        <Box sx={{ flex: "1 1 480px", minWidth: 0 }}>
          {/* Header card */}
          <Card
            sx={{ p: 3, mb: 2, bgcolor: PAPER, border: `1px solid ${BORDER}` }}
          >
            <Box
              sx={{
                display: "flex",
            ...
            {ticket.resolution_note && (
              <Box
                sx={{
                  ...
                  p: 2,
                  borderRadius: 1.5,
                  bgcolor: "rgba(90,143,114,0.08)",
                  border: "1px solid rgba(90,143,114,0.3)",
                }}
              >
                <Typography
                  sx={{
           ...
           {(canSubmitFeedback || hasFeedback) && (
            <Card sx={{ p: 2.5, mb: 2, bgcolor: PAPER, border: `1px solid ${BORDER}` }}>
              ...
              {hasFeedback ? (
                <Box>
                  <StarRating value={ticket.feedback_rating} onChange={() => {}} readOnly />
                  {ticket.feedback_comment && (
                    <Typography sx={{ fontSize: 13, color: TEXT_DIM, lineHeight: 1.6, mt: 1, whiteSpace: 'pre-wrap' }}>
                      {ticket.feedback_comment}
                    </Typography>
                  )}
                  <Typography sx={{ fontSize: 11, color: TEXT_MUTED, mt: 1 }}>
                    Submitted {formatDateTime(ticket.feedback_submitted_at)}
                  </Typography>
                </Box>
              ) : (
                <Button
                  ...
                  startIcon={<span className="material-symbols-outlined" style={{ fontSize: 15 }}>star</span>}
              ...
              <Box sx={{ display: "flex", gap: 1, flexWrap: "wrap" }}>
                ...
                {ticket.ticket_status === "in_progress" && (
                  <Button
                    ...
                    size="small"
                    onClick={() =>
                      setNoteDialog({ open: true, mode: "struggling" })
                    }
                    startIcon={
                      <span
                        className="material-symbols-outlined"
                    ...
                    sx={{
                      fontSize: 12,
                      color: "#8b5e6a",
                      borderColor: "rgba(139,94,106,0.4)",
                    }}
                  >
                    Flag as Struggling
                  </Button>
                ...
                <Button
                  ...
                  color="success"
                  onClick={() =>
                    setNoteDialog({ open: true, mode: "resolved" })
                  }
                  startIcon={
                    <span
                      className="material-symbols-outlined"
          ...
          {canReopen && (
            ...
            >
              ...
              >
                This ticket is {isClosed ? "closed" : "resolved"}. You can
                reopen it if the issue has recurred or the resolution was
                incomplete.
              </Typography>
              <Button
                variant="contained"
          ...
          {canEscalate && (
            ...
            >
              ...
              >
                Flag this ticket for urgent attention before it breaches SLA
                automatically.
              </Typography>
              <Button
                variant="contained"
                onClick={() => setNoteDialog({ open: true, mode: "escalate" })}
                startIcon={
                  <span
                    className="material-symbols-outlined"
                    style={{ fontSize: 15 }}
  ...
  >
    ...
    >
        ...
        <Box sx={{ flex: "0 0 260px" }}>
          <Card sx={{ p: 2.5, bgcolor: PAPER, border: `1px solid ${BORDER}` }}>
            ...
            <InfoRow label="Category">
              <Typography
                sx={{ fontSize: 12.5, color: TEXT_BRIGHT, fontWeight: 600 }}
              >
              ...
              >
                {ticket.department_name ??
                  (ticket.department_id
                    ? String(ticket.department_id)
                    : "Unrouted")}
              </Typography>
            </InfoRow>
            <InfoRow label="Submitted">
              <Typography sx={{ fontSize: 12, color: TEXT_BRIGHT }}>
                {timeAgo(ticket.ticket_created_at)}
              </Typography>
            </InfoRow>
            <InfoRow label="Updated">
              <Typography sx={{ fontSize: 12, color: TEXT_BRIGHT }}>
                {timeAgo(ticket.ticket_updated_at)}
              </Typography>
            </InfoRow>
            ...
            <InfoRow label="Attachment">
              {ticket.attachment_id ? (
                ...
                >
                  ...
                  {ticket.attachment_file_name?.length > 18
                    ? `${ticket.attachment_file_name.slice(0, 15)}…`
            ...
            <InfoRow label="Assigned to">
              {assigneeDisplay ? (
                <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                  ...
                  <Typography
                    sx={{ fontSize: 12, color: "#5a8dc4", fontWeight: 600 }}
                ...
                <Typography
                  sx={{ fontSize: 12, color: "#c49a4a", fontWeight: 500 }}
                >
                  Unassigned
                </Typography>
              )}
      ...
      <Dialog
        ...
        PaperProps={{ sx: { bgcolor: PAPER, border: `1px solid ${BORDER}`, borderRadius: 2, minWidth: { xs: '92vw', sm: 420 } } }}
      >
        <DialogTitle sx={{ color: TEXT_BRIGHT, fontWeight: 700, pb: 1 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            <span className="material-symbols-outlined" style={{ color: '#c49a4a', fontSize: 20, fontVariationSettings: "'FILL' 1" }}>star</span>
            Leave feedback
          </Box>
        </DialogTitle>
        <DialogContent>
          <Typography sx={{ color: TEXT_DIM, fontSize: 13, mb: 2 }}>
            How was your experience with this ticket's resolution?
          </Typography>
          <Box sx={{ mb: 2 }}>
          ...
          <TextField
            ...
            sx={{
              '& .MuiOutlinedInput-root': {
                ...
                '& fieldset': { borderColor: 'rgba(143,162,192,0.2)' },
                '&:hover fieldset': { borderColor: 'rgba(143,162,192,0.4)' },
                '&.Mui-focused fieldset': { borderColor: '#c49a4a' },
              },
            }}
          />
        </DialogContent>
        <DialogActions sx={{ px: 3, pb: 2.5, gap: 1 }}>
          <Button onClick={() => setFeedbackDialog(false)} variant="outlined" sx={{ color: TEXT_DIM, borderColor: 'rgba(143,162,192,0.2)' }}>
            Cancel
          </Button>
          <Button
            ...
            variant="contained"
            sx={{ bgcolor: '#c49a4a', color: '#0a1628', '&:hover': { bgcolor: '#b88a3a' } }}
            startIcon={submittingFeedback ? <CircularProgress size={13} sx={{ color: '#0a1628' }} /> : <span className="material-symbols-outlined" style={{ fontSize: 16 }}>check</span>}
          >
