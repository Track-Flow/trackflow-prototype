...
// Oldest created_at first — FIFO. Never re-sorts on status move.
...
function cardLockState(ticket, userId) {
  ...
  const draggable      = isOwned && !isClosed && !isResolved;
  return { isClaimed, isOwned, isOtherOwned, isClosed, isResolved, draggable };
}

// ─── Generic note-required dialog (used for both Resolve and Struggling) ──────
function NoteDialog({ open, mode, onConfirm, onCancel }) {
  ...
  const config = isResolve
    ? {
        ...
        helper: "Please provide resolution notes before marking this task as resolved.",
    ...
    >
      <DialogTitle sx={{ color: "#e6edf7", fontWeight: 700, pb: 1 }}>
        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
          <span className="material-symbols-outlined" style={{ color: config.iconColor, fontSize: 20 }}>
            {config.icon}
          </span>
          {config.title}
        </Box>
...
function ClaimFirstBadge() {
  return (
    <Tooltip title="Claim this ticket before you can drag it" arrow>
      <Box
        sx={{
          display: "inline-flex", alignItems: "center", gap: 0.4,
          px: 0.75, py: 0.2, borderRadius: 1,
          bgcolor: "rgba(255,181,71,0.15)", border: "1px solid rgba(255,181,71,0.45)",
          cursor: "help",
        }}
      >
        <span className="material-symbols-outlined" style={{ fontSize: 12, color: "#ffb547" }}>lock</span>
        <Typography sx={{ fontSize: 8.5, fontWeight: 800, color: "#ffb547", letterSpacing: "0.04em" }}>
          CLAIM FIRST
        </Typography>
      </Box>
    </Tooltip>
...
// ─── "Closes in Xh Ym" badge for resolved tickets ────────────────────────────
    ...
    <Tooltip title="Auto-closes 24 hours after resolution" arrow>
      <Box
        sx={{
          ...
          bgcolor: "rgba(71,85,105,0.25)", border: "1px solid rgba(71,85,105,0.5)",
          cursor: "help",
        }}
      >
        <span className="material-symbols-outlined" style={{ fontSize: 10, color: "#94a3b8" }}>schedule</span>
        <Typography sx={{ fontSize: 9.5, fontWeight: 700, color: "#94a3b8", letterSpacing: "0.02em" }}>
          {label}
        </Typography>
      </Box>
    </Tooltip>
    ...
    <Box
      ...
      sx={{
        ...
        border: `1px solid ${
          ...
          : isResolved ? "rgba(43,212,143,0.2)"
          : !isClaimed ? "rgba(255,181,71,0.3)"
          : isStruggling ? "rgba(255,107,107,0.3)"
          : isDragging ? ACCENT
          : BORDER
        }`,
        cursor: draggable ? "grab" : "pointer",
        opacity: isClosed ? 0.55 : isResolved ? 0.65 : 1,
        position: "relative",
        overflow: "hidden",
        transition: "all .15s",
        "&:hover": !isClosed ? {
          border: `1px solid ${
            isResolved ? "rgba(43,212,143,0.35)"
            : !isClaimed ? "rgba(255,181,71,0.5)"
            : isOwned ? "rgba(46,200,255,0.35)"
        ...
        <Box sx={{ display: "flex", alignItems: "center", gap: 0.75 }}>
          {isClosed ? (
            <Tooltip title="Closed — locked" arrow>
              <span className="material-symbols-outlined" style={{ fontSize: 13, color: "#475569" }}>lock</span>
            </Tooltip>
          ) : isResolved ? (
            <>
          ...
          {isStruggling && (
            <Tooltip title="Flagged as struggling" arrow>
              <span className="material-symbols-outlined" style={{ fontSize: 13, color: "#ff6b6b" }}>flag</span>
            </Tooltip>
          )}
      ...
      <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", pl: 0.5 }}>
        {isClaimed ? (
          ...
          <Button
            ...
            startIcon={
              isClaiming
                ? <CircularProgress size={11} sx={{ color: "#ffb547" }} />
            ...
            sx={{
              ...
              color: "#ffb547", bgcolor: "rgba(255,181,71,0.12)",
              border: "1px solid rgba(255,181,71,0.4)",
              "&:hover": { bgcolor: "rgba(255,181,71,0.22)", borderColor: "#ffb547" },
            }}
          >
...
// ─── Column ───────────────────────────────────────────────────────────────────
function KanbanColumn({ col, tickets, draggingId, claimingId, onDragStart, onDrop, onDragOver, onCardClick, onClaim, userId }) {
        ...
        {tickets.length === 0 && (
          <Box sx={{ p: 2, textAlign: "center", borderRadius: 1.5, border: `1px dashed ${BORDER}`, mt: 0.5 }}>
            <Typography sx={{ fontSize: 12, color: "#3a4f6a" }}>
              {isClosed ? "Closes automatically 24h after resolution" : "No tickets"}
            </Typography>
          </Box>
        )}
        {tickets.map((t) => (
...
export default function TLABoard() {
  ...
  // noteDialog.mode is either "resolved" or "struggling" — both use the same required-note flow
  ...
  const fetchTickets = async () => {
    try {
      ...
      // Show unclaimed AND claimed tickets in this department — closed included so it has a home column
  ...
  const handleClaim = async (ticketId) => {
    ...
    try {
      ...
      // refetch to get the server's authoritative assignee_name/join data
      fetchTickets();
    } catch (err) {
      // roll back on failure (e.g. someone else claimed it first — 409)
      setTickets((prev) =>
        prev.map((t) => t.ticket_id === ticketId
  ...
  const handleDrop = async (e, targetStatus) => {
    ...
    if (!DROPPABLE.includes(targetStatus)) return; // closed is never a manual drop target
    ...
    // Both resolved and struggling require a note before the status changes
  ...
  // Oldest created_at first, per column — FIFO, never resorts on status change
  ...
  return (
    <Box sx={{ p: 3, height: "100%", display: "flex", flexDirection: "column" }}>
      ...
      {/* Stats strip */}
      <Box sx={{ display: "flex", gap: 2, mb: 2.5, flexShrink: 0, flexWrap: "wrap" }}>
        {COLUMNS.map((col) => (
      ...
      {!loading && (
        <Typography sx={{ fontSize: 11, color: "#3a4f6a", textAlign: "center", mt: 1.5, flexShrink: 0 }}>
          Claim a ticket to start working on it · Only claimed tickets can be dragged · Closed tickets are locked
        </Typography>
      )}
